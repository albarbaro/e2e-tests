...
//
func (h *SuiteController) ExecuteImportCommand(ManagedClusterKubeconfig string, importCommand string) error {
	// TDB
	// Should this run inside a pod by itself?

	cmd, err := exec.Command("bash", "-c", "KUBECONFIG="+ManagedClusterKubeconfig+" && "+importCommand).CombinedOutput()
	if err != nil {
		return err
	}
